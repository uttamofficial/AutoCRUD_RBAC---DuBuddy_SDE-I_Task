...
const result1 = validateModelDefinition(employeeModel);
...
const result2 = validateModelDefinition({
  name: 'invalid_model',
...
const result3 = validateModelDefinition({
  name: 'TestModel',
  fields: [
    { name: 'id', type: 'number' },
    { name: 'id', type: 'string' }
...
const result4 = validateModelDefinitions(exampleModels);
...
const result5 = validateModelDefinitions([
  {
    name: 'Task',
    fields: [
      { name: 'id', type: 'number' },
...
const test6a = hasPermission(rbac, 'Admin', 'delete') === true;
const test6b = hasPermission(rbac, 'Viewer', 'delete') === false;
const test6c = hasPermission(rbac, 'Manager', 'read') === true;
...
const result7 = validateModelDefinition(JSON.parse(jsonModel));
