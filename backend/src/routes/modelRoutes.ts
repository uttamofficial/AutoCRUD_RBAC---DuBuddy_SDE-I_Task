...
let modelCache: Map<string, { model: ModelDefinition; lastModified: number }> = new Map();
 ...
 */
async function saveVersionedModel(model: ModelDefinition): Promise<number> {
  await ensureDirectories();
  ...
  const versionFile = path.join(VERSIONS_DIR, `${model.name}_v${nextVersion}.json`);
  await fs.writeFile(versionFile, JSON.stringify(versionedModel, null, 2), 'utf-8');
...
router.post('/save', async (req: Request, res: Response) => {
  try {
    ...
    const validationResult = validateModelDefinition(modelJson);
    ...
    await fs.writeFile(
      filePath,
      JSON.stringify(modelWithMetadata, null, 2),
      'utf-8'
    );
    ...
    await writeAuditLog(
      ...
      undefined,
      'system',
      { version, fieldCount: model.fields.length }
    );
...
router.get('/:name', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const validationResult = validateModelDefinition(model);
...
router.get('/:name/versions', async (req: Request, res: Response) => {
  try {
    ...
    const versionFiles = await fs.readdir(VERSIONS_DIR);
    const modelVersions = versionFiles.filter(f => 
      f.startsWith(`${name}_v`) && f.endsWith('.json')
    );
    
    const versions = await Promise.all(
      modelVersions.map(async (file) => {
        try {
 ...
 */
router.get('/:name/versions/:version', async (req: Request, res: Response) => {
  try {
    const { name, version } = req.params;
    const versionFile = path.join(VERSIONS_DIR, `${name}_v${version}.json`);
...
router.get('/audit-logs', async (req: Request, res: Response) => {
  try {
    ...
    let logFiles = await fs.readdir(AUDIT_DIR);
    logFiles = logFiles.filter(f => f.startsWith('audit-') && f.endsWith('.json'));
    
    // Filter by date if provided
    if (date) {
      logFiles = logFiles.filter(f => f.includes(date as string));
    }
